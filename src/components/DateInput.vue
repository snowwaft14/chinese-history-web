  ...
  <div class="date-input-component w-full">
    ...
    <template v-if="isEraType">
      <div class="flex flex-wrap items-center gap-1 w-full">
        <SearchableSelect v-model="dynastyName" :options="dynastyOptions" placeholder="唐" value-key="value"
          label-key="label" search-key="searchValues" input-class="input input-bordered input-sm text-left text-xs w-18"
          @change="onDynastyChange" @expand="onDynastyExpand" />

        <SearchableSelect v-model="emperorId" :options="emperorOptions" placeholder="唐玄宗" value-key="value"
          label-key="label" search-key="searchValues" input-class="input input-bordered input-sm text-left text-xs w-27"
          @change="onEmperorChange" />

        <SearchableSelect v-model="eraName" :options="eraOptions" placeholder="天宝" value-key="value" label-key="label"
          search-key="searchValues" input-class="input input-bordered input-sm text-left text-xs w-24"
          @change="onEraChange" />

        <SearchableSelect v-model="eraYear" :options="eraYearOptions" placeholder="十四载" value-key="value"
          label-key="label" search-key="searchValues" input-class="input input-bordered input-sm text-left text-xs w-24"
          @change="onEraYearChange" />

        <SearchableSelect :model-value="monthValue" :options="lunarMonthOptions" placeholder="冬月" value-key="value"
          label-key="label" search-key="searchValues" input-class="input input-bordered input-sm text-left text-xs w-18"
          @change="onMonthChange" />

        <SearchableSelect :model-value="dayValue" :options="lunarDayOptions" placeholder="初九" value-key="value"
          label-key="label" search-key="searchValues" input-class="input input-bordered input-sm text-left text-xs w-18"
          @change="onDayChange" />
      </div>
    </template>
    ...
    <template v-else>
      <div class="flex items-center gap-2 w-full">
        ...
        <div class="flex items-center gap-1">
          <input type="number" :value="yearValue" @input="onYearChange" @focus="handleInputFocus" placeholder="755"
            min="1" :max="yearMax" class="input input-bordered input-sm text-center w-20" />
          <span class="text-sm font-medium text-base-content">年</span>
        </div>

        <!-- 月份输入 -->
        <div class="flex items-center gap-1">
          <input v-if="isGregorianType" type="number" :value="monthValue" @input="onMonthChange"
            @focus="handleInputFocus" placeholder="12" min="1" max="12"
            class="input input-bordered input-sm text-center w-20" />
          <SearchableSelect v-else :model-value="monthValue" :options="lunarMonthOptions" placeholder="冬月"
            value-key="value" label-key="label" search-key="searchValues"
            input-class="input input-bordered input-sm text-left text-xs w-21" @change="onMonthChange" />
          <span v-if="isGregorianType" class="text-sm font-medium text-base-content">月</span>
        </div>

        <!-- 日期输入 -->
        <div class="flex items-center gap-1">
          <input v-if="isGregorianType" type="number" :value="dayValue" @input="onDayChange" @focus="handleInputFocus"
            placeholder="16" min="1" max="31" class="input input-bordered input-sm text-center flex-1 w-20" />
          <SearchableSelect v-else :model-value="dayValue" :options="lunarDayOptions" placeholder="初九" value-key="value"
            label-key="label" search-key="searchValues"
            input-class="input input-bordered input-sm text-left text-xs w-21" @change="onDayChange" />
  ...
  <div class="flex items-center gap-4" v-if="hasOptions">
    ...
    <div v-if="isGregorianType" class="form-control w-fit">
      <label class="label cursor-pointer p-0 gap-2">
        <input type="checkbox" v-model="isBC" @change="onBCChange" class="toggle toggle-sm toggle-primary" />
    ...
    <div v-if="!isGregorianType" class="form-control w-fit">
      <label class="label cursor-pointer p-0 gap-2">
        <input type="checkbox" v-model="isLeapMonth" @change="onLeapMonthChange" class="toggle toggle-sm toggle-primary" />
...
const onEmperorChange = async (value: string | undefined) => {
  ...
  if (value) {
    try {
      ...
      // 自动选择该皇帝的第一个年号（按照startDate最早的）
      if (eras.value.length > 0) {
        const sortedEras = eras.value
          .filter(era => era.startDate) // 过滤有startDate的年号
          .sort((a, b) => {
        ...
        if (sortedEras.length > 0) {
          const firstEra = sortedEras[0];
          eraName.value = firstEra.name;
          eraYear.value = 1; // 默认元年
          // 月和日保持undefined，表示该年号元年正月初一
          console.log(`自动选择皇帝 ${value} 的第一个年号: ${firstEra.name}`);

          // 获取年号详情
          if (dynastyName.value) {
            try {
              currentEraDetail.value = await GlobalCacheInstances.getEraDetail(dynastyName.value, firstEra.name);
            } catch (error) {
              console.error(`获取年号 ${firstEra.name} 详情失败:`, error);
            }
          }
        }
      }
