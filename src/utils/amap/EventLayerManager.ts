 ...
 * 事件图层管理器 - 简化版本
 */
export class EventLayerManager extends LayerManagerBase {
  constructor(map: AMap.Map) {
    super(map, LayerType.EVENT);
  }
    ...
    return ({ marker }) => {
      try {
        ...
        const icon = new (window as any).AMap.Icon({
          image: this.createEventIconSvg('#52c41a', 24),
  ...
  createRenderClusterMarker(): ({ count, marker }: { count: number; marker: AMap.Marker }) => void {
    return ({ count, marker }) => {
      try {
        ...
        console.error('渲染事件聚合标记失败:', error);
      }
    };
  }
   ...
   * 创建事件图标SVG - 使用预定义图标
   */
  private createEventIconSvg(color: string, size: number): string {
    // 使用预定义的事件图标，忽略传入的颜色参数，使用原始设计
  ...
  createClusterMarker(context: import('./amap').MarkerClusterContext): void {
    try {
      const factor = 1;
      const hue = 120; // 绿色色调，适合事件
      const bgColor = `hsla(${hue}, 60%, 45%, 0.8)`;
      const fontColor = `hsla(${hue}, 100%, 95%, 1)`;
      const borderColor = `hsla(${hue}, 60%, 35%, 1)`;
      ...
      console.error('创建事件聚合标记失败:', error);
    }
  }
  ...
  private createClusterIconSvg(color: string, size: number, text: string): string {
    ...
    const svg = `
      <svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 ${size} ${size}">
        <image href="/icons/event_cluster.svg" width="${size}" height="${size}"/>
